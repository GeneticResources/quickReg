...
plot.reg <- function(x, limits = c(NA, NA), sort = "order", title=NULL,remove=TRUE,...) {
  var_args<-names(match.call(expand.dots=TRUE)[-1])
    if (! "reg" %in% class(x)  ) {
      if(! all(c("term","center","lower","high", "model") %in% var_args )) {
        stop("Not `reg` object, please provide `term`,`center`,`lower`, `high` and `model` for plot.", call. = FALSE)
      }
    }
