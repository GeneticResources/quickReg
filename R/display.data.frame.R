...
#' @param col Column indices or names of the variables in the dataset to display, the default columns are all the variables
...
display.data.frame <- function(x = NULL, col = NULL, normtest = NULL,  useNA = "ifany", discrete_limit = 10, exclude_discrete=TRUE, ...) {
    ...
    for (i in col) {
        ...
        if (length(unique(one)) >=  discrete_limit & is.numeric(one)) {
            summary <- summary(one)
            describe <- psych::describe(one)[2:13]
            row.names(describe)<-""

            normality <- do.call(normtest,list(one))
        ...
        } else if (length(unique(one)) >= discrete_limit&&exclude_discrete) {
          result[[term]] <- list(split_line = split_line, warning=paste0("`",term,"`"," is verbose. To display it using `exclude_discrete = FALSE` "))
            ...
            if (useNA != "no") {
                if (any(is.na(one))) {
                  count_NA <- table(one, useNA = useNA)
